...
// Handle Home button redirect using hasRole() function
if(isset($_GET['home'])) {
    if(hasRole('admin')) {
        ...
        exit();
    } elseif(hasRole('report')) {
        // Stay on this page, just remove the query parameter
        header('Location: ' . str_replace('?home=1', '', $_SERVER['REQUEST_URI']));
        exit();
    } else {
...
// Store chart data as numbers safely
$completedAssignmentsNum = (int)$completed_assignments;
$pendingAssignmentsNum = (int)$pending_assignments;
?>
...
<nav class="top-navbar">
    ...
    <div style="display: flex; align-items: center; gap: 20px;">
        ...
        <div style="display: flex; gap: 10px; align-items: center;">
            <a href="?home=1" class="btn-indigo" style="width: auto; padding: 8px 16px; border-radius: 8px; font-size: 13px; display: flex; align-items: center; gap: 6px; font-weight: 600; text-transform: none; letter-spacing: 0; text-decoration: none; cursor: pointer;">
                <span class="material-symbols-outlined" style="font-size: 18px;">assessment</span> Home
            </a>
            <a href="../logout.php" class="logout-btn" style="padding: 6px 12px;"><span class="material-symbols-outlined">logout</span></a>
        </div>
    </div>
</nav>
    ...
    <div class="grid-2">
        ...
        <div class="card" style="display: flex; flex-direction: column;">
            ...
            <div class="table-responsive" style="flex: 1;">
                <table>
                    <thead>
                        <tr><th>Date</th><th>Agent</th><th>Store Location</th><th>Status</th></tr>
                    </thead>
                    <tbody>
                        <?php if(empty($recent_assignments)): ?>
                            <tr><td colspan="4" style="text-align: center; color: var(--text-secondary); padding: 40px 0;">No recent dispatches found.<?php echo ' '; ?></td></tr>
                        <?php else: ?>
                            ...
                            <tr>
                                ...
                                <td><span class="badge badge-<?php echo $ra['status']; ?>"><?php echo ucfirst($ra['status']); ?></span></td>
                            </tr>
                            <?php endforeach; ?>
                        <?php endif; ?>
                    </tbody>
...
<div id="accessModal" class="modal-overlay">
    <div class="modal-card">
        <span class="material-symbols-outlined modal-icon">gpp_bad</span>
        <div class="modal-title">Access Restricted</div>
        <div class="modal-text">You do not have administrative clearance parameters. Please contact your IT Systems Administrator for credential elevation.</div>
        <button onclick="document.getElementById('accessModal').classList.remove('show')" class="btn btn-white" style="padding: 10px 20px; font-size: 12px; width: auto; display: inline-flex; margin: 0 auto;">Dismiss Window</button>
    </div>
</div>

<script>
    // Store Directory for filtering
    ...
    if(agentData.length > 0) {
        new Chart(document.getElementById('agentChart'), {
            ...
            data: {
                ...
                datasets: [{ data: agentData.map(d => d.completed_count), backgroundColor: 'rgba(16, 185, 129, 0.9)', borderRadius: 4 }]
            },
    ...
    if(regionData.length > 0) {
        new Chart(document.getElementById('regionChart'), {
            ...
            data: {
                ...
                datasets: [{ data: regionData.map(d => d.assign_count), backgroundColor: ['#6366f1', '#8b5cf6', '#ec4899', '#f43f5e', '#f59e0b'], borderWidth: 2, borderColor: '#171931' }]
            },
    ...
    // Status Chart - Using the safe number variables
    const completedVal = <?php echo $completedAssignmentsNum; ?>;
    const pendingVal = <?php echo $pendingAssignmentsNum; ?>;
    
    new Chart(document.getElementById('statusChart'), {
        type: 'doughnut',
        data: {
            labels: ['Completed', 'Pending'],
            datasets: [{ data: [completedVal, pendingVal], backgroundColor: ['rgba(16, 185, 129, 0.9)', 'rgba(245, 158, 11, 0.9)'], borderWidth: 2, borderColor: '#171931' }]
        },
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'right', labels: { boxWidth: 10, font: { size: 10 } } } }, cutout: '75%' }
    });
</script>
