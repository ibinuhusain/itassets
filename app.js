...
window.renderDispatchSearchResults = function(items) { 
    ...
    if(!container) return; 
    if(items.length === 0) { container.innerHTML = '<p style="padding: 15px; font-weight: 600; color: #64748b; text-transform: uppercase;">ZERO MATCHES.</p>'; return; } 
    ...
    container.innerHTML = items.map(item => { 
        ...
        return `<div style="padding: 15px; border-bottom: 1px solid #e2e8f0; background: #fff; display: flex; justify-content: space-between; align-items: center;"><div><strong style="font-size: 13px;">${item.item_name || item.name}</strong> <span style="font-family:monospace; margin-left: 5px; color: #64748b;">(${item.sku})</span> ${warningBadge}<br><span style="font-size:11px; color:#64748b; font-weight: 500; margin-top: 4px; display: inline-block;">BALANCE: ${item.quantity_on_hand || item.qty} UNITS | CAT: ${item.category || '-'}</span></div><button class="btn-cancel" onclick="addToDispatchCart('${item.sku}')"><i class="fas fa-plus"></i> ADD</button></div>`; 
    }).join(''); 
}
