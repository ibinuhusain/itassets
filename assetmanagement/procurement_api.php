...
if ($method === 'POST') {
    ...
    try {
        ...
        // --- NEW: CREATE STORE ENDPOINT ---
        if ($action === 'create_store') {
            // Using sCode as the primary 'id' for the store table
            $stmt = $conn->prepare("INSERT INTO stores (id, name, brand, brand_code, mall, entity, city, country, address, region_id, email, route_code, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1)");
            ...
            $stmt->execute();
            logAction($conn, $user, 'CREATE_STORE', "Registered new store: " . $data['sName'] . " [" . $data['sCode'] . "]");
            
            echo json_encode(["status" => "success"]); 
            exit;
        }

        // --- NEW: DISABLE STORE ENDPOINT (For Reverse Logistics) ---
        if ($action === 'disable_store') {
            $storeId = $_GET['id'] ?? '';
            $stmt = $conn->prepare("UPDATE stores SET is_active = 0 WHERE id = ?");
            $stmt->bind_param("s", $storeId);
            $stmt->execute();
            
            logAction($conn, $user, 'DISABLE_STORE', "Disabled Store ID via Reverse Logistics: " . $storeId);
