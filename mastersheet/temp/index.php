...
<head>
    ...
    <style>
        /* Enterprise Dark Theme Integration Tokens */
        :root {
            --bg-card: #13151a;
        ...
        .sidebar { 
            width: 280px; 
            height: 100vh;
            background-color: rgba(23, 25, 49, 0.95); 
            color: #fff; 
            display: flex; 
        ...
        /* Full Widescreen Dynamic Column Grids */
        ...
        /* High-Density Bound Sticky Scroll Tables */
        ...
        .table tbody tr {
            background-color: rgba(28, 31, 38, 0.4) !important;
        }
        .table tbody td { 
        ...
        /* Combined Layout Dynamic Navigation Subtabs */
        .mapping-tabs {
            display: flex;
            gap: 8px;
            border-bottom: 1px solid var(--border-color);
        ...
        /* Dual Listbox Layout Configuration */
        ...
        @media (max-width: 768px) {
            .dual-listbox-container { grid-template-columns: 1fr; }
            .control-action-arrows { flex-direction: row !important; }
        }
...
<body>
    ...
    <script>
        // Directly inherit context from the secure PHP Session instead of localStorage
        const currentRole = "<?php echo $primary_role; ?>";
        const currentUserName = "<?php echo $username; ?>";
        
        // App users don't have desktop matrix views, route them out if they force the URL
    ...
    <script>
        ...
        let allUsers = [], currentUsersPage = 1, usersSearchTerm = '';
        ...
        function renderMenu() {
            ...
            // If role is undefined in menus object, trigger universal logout
        ...
        function navigate(viewId, element) {
            ...
            // --- VIEW: MASTER DATA UPLOAD ---
                ...
                container.innerHTML = `
                    <div class="panel-card" style="max-width: 900px; margin: 0 auto;">
                        <h3><span class="material-symbols-outlined" style="color:var(--accent-blue)">upload_file</span> Data Injection Port</h3>
                        <div id="uploadAlert" class="alert d-none"></div>
                        
                        <form id="uploadForm" onsubmit="processFileLocally(event)">
                            <div class="row g-3">
                                <div class="col-md-6">
                                    <label class="form-label">Target Destination Merchant Profile</label>
                                    <select id="adminUploadMerchant" class="form-select" required>
                                        <option value="" disabled selected>Loading merchant entries...</option>
                                    </select>
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label">Select Master Spreadsheet Source (.csv, .xlsx)</label>
            ...
            // --- VIEW: COMBINED ASSIGNMENTS HUB (MERCHANT & AGENT) ---
            ...
            // --- VIEW: USER MANAGEMENT ---
                ...
                title.innerText = "User Management Console";
                container.innerHTML = `
                    <div class="mapping-workspace-grid">
                        <div class="panel-card">
                            <h3><span class="material-symbols-outlined" style="color:var(--accent-blue)">person_add</span> Provision Profile</h3>
                            <div id="userAlert" class="alert d-none"></div>
                            <form id="createUserForm" onsubmit="createUser(event)">
                                <div class="form-group mb-3"><label class="form-label">Username Handle</label><input type="text" id="newUsername" class="form-control" placeholder="e.g. agent55" required></div>
                                <div class="form-group mb-3"><label class="form-label">Security Password</label><input type="password" id="newPassword" class="form-control" placeholder="••••••••" required></div>
                                <div class="form-group mb-4">
                                    <label class="form-label">System Security Group Role</label>
                                    <select id="newRole" class="form-select" required>
                                        <option value="" disabled selected>Select Role Matrix...</option>
                                        <option value="admin">System Administrator</option>
                                        <option value="merchant">Merchant Group Owner</option>
                                        <option value="app_user">Field Auditor (Agent)</option>
                                    </select>
                                </div>
                                <button type="submit" id="createUserBtn" class="btn btn-primary w-100">Create Access Credentials</button>
                            </form>
                        </div>
                        
                        <div class="panel-card">
                            <div class="table-header-bar">
                                <h2>Active Security Access Logs</h2>
                                <div class="search-wrapper">
                                    <span class="material-symbols-outlined">search</span>
                                    <input type="text" class="search-input" placeholder="Search profiles..." onkeyup="handleUserSearch(this.value)">
                                </div>
                            </div>
                            <div class="table-scroll-container">
                                <table class="table">
                                    <thead><tr><th style="width:60px;">ID</th><th>User Reference Handle</th><th>Security Group</th><th>State Status</th><th style="text-align:right;">Security Controls</th></tr></thead>
                                    <tbody id="userTableBody"><tr><td colspan="5" class="text-center py-4">Loading core identities...</td></tr></tbody>
                                </table>
                            </div>
                            <div class="d-flex justify-content-between align-items-center mt-3">
                                <small id="userPageInfo" class="text-muted font-monospace" style="font-size:12px;"></small>
                                <div class="d-flex gap-2">
                                    <button class="btn btn-outline-secondary bg-white btn-sm" style="padding:2px 10px;" onclick="changeUserPage(-1)">&lt; Prev</button>
                                    <button class="btn btn-outline-secondary bg-white btn-sm" style="padding:2px 10px;" onclick="changeUserPage(1)">Next &gt;</button>
                                </div>
                            </div>
                        </div>
                    </div>`;
            ...
            // --- VIEW: MERCHANT WORKSPACE ONLY OVERLAY ---
                ...
                container.innerHTML = `
                    ...
                    <div class="panel-card">
                        <h3><span class="material-symbols-outlined" style="color:var(--accent-blue)">cloud_upload</span> Stage Updated Log Asset</h3>
                        <div id="uploadAlert" class="alert d-none"></div>
                        <form id="uploadForm" onsubmit="processFileLocally(event)">
                    ...
                    <div class="panel-card">
                        ...
                        <div class="table-scroll-container">
                            <table class="table">
                                <thead><tr><th>Item Barcode ID</th><th>Description Field Specification</th><th>Location Space</th><th>Pricing Level Mapping (Was/Now)</th><th>Expiration Metric</th></tr></thead>
                                <tbody id="itemsTableBody"><tr><td colspan="5" class="text-center py-4">Waiting on stack execution loop...</td></tr></tbody>
                            </table>
                        </div>
                        <div class="d-flex justify-content-between align-items-center mt-3">
        ...
        async function fetchUsers() {
            try {
                ...
                const result = await response.json();
                if (result.success) { allUsers = result.data; renderUsers(); }
            } catch (e) {}
        }

        function handleUserSearch(term) { usersSearchTerm = term.toLowerCase(); currentUsersPage = 1; renderUsers(); }
        function changeUserPage(dir) { currentUsersPage += dir; renderUsers(); }

        function renderUsers() {
            const filtered = allUsers.filter(u => u.username.toLowerCase().includes(usersSearchTerm) || u.role.toLowerCase().includes(usersSearchTerm));
            const totalPages = Math.ceil(filtered.length / itemsPerPage) || 1;
            ...
            if(pageData.length === 0) {
                html = '<tr><td colspan="5" class="text-center py-4 text-muted">No users found matching search fields.</td></tr>';
            } else {
                pageData.forEach(user => {
                    ...
                    html += `<tr>
                        ...
                        <td><strong class="text-white">${user.username}</strong></td>
                        <td><span class="badge-role">${displayRole}</span></td>
                        <td><span class="badge-status">${user.status}</span></td>
                        <td>
                            <div class="d-flex gap-2 justify-content-end">
                                <button class="btn-action-warning" onclick="resetUserPassword(${user.id}, '${user.username}')">Reset Key</button>
                                <button class="btn-action-delete ${isMe ? 'disabled' : ''}" onclick="deleteUser(${user.id}, '${user.username}')">Evict</button>
                            </div>
                        </td>
                    </tr>`;
                });
            ...
            document.getElementById('userPageInfo').innerText = `Page ${currentUsersPage} of ${totalPages} (Total Profiles: ${filtered.length})`;
        }

        async function createUser(event) {
            event.preventDefault();
            const payload = { username: document.getElementById('newUsername').value, password: document.getElementById('newPassword').value, role: document.getElementById('newRole').value };
            try {
                const response = await fetch('api/users.php', { method: 'POST', body: JSON.stringify(payload) });
                const result = await response.json();
                showTempAlert('userAlert', result.message, result.success);
                if (result.success) { document.getElementById('createUserForm').reset(); fetchUsers(); }
            } catch (e) { showTempAlert('userAlert', "System network dropped", false); }
        }

        async function deleteUser(userId, username) {
            if(!confirm(`Are you certain you want to destroy permission token logs for user account line: '${username}'?`)) return;
            try {
                const response = await fetch('api/users.php', { method: 'POST', body: JSON.stringify({ action: 'delete_user', user_id: userId }) });
                const result = await response.json();
        ...
        async function resetUserPassword(userId, username) {
            const newPassword = prompt(`Assign absolute reset credential password sequence for user account [ ${username} ] :`);
            if (!newPassword) return;
            try {
                const response = await fetch('api/users.php', { method: 'POST', body: JSON.stringify({ action: 'reset_password', user_id: userId, new_password: newPassword }) });
        ...
        async function fetchMappings() {
            try {
                ...
                if(result.success) {
                    ...
                    // RESOLUTION: Dynamic mapping inside dropdown blocks showing profile names beside usernames
                    ...
                    if(merchSelect) {
                        merchSelect.innerHTML = '<option value="" disabled selected>Choose target profile links...</option>' + 
                            rawMerchants.map(m => {
                                const resolvedName = m.name ? `${m.name} (@${m.username})` : `@${m.username}`;
                    ...
                    if(agentSelect) {
                        agentSelect.innerHTML = '<option value="" disabled selected>Choose target agent links...</option>' + 
                            rawAppUsers.map(u => {
                                const resolvedName = u.name ? `${u.name} (@${u.username})` : `@${u.username}`;
        ...
        function renderAgentDualListBox() {
            ...
            allBrands.forEach(brand => {
                if(assignedBrandIds.includes(brand.id)) {
                ...
                if(result.success && result.merchants) {
                    ...
                    if(select) {
                        select.innerHTML = '<option value="" disabled selected>Select merchant endpoint...</option>' + 
                            result.merchants.map(m => `<option value="${m.username}">${m.name ? m.name + ' (@' + m.username + ')' : m.username}</option>`).join('');
                    }
                }
            } catch (e) {}
        }
            ...
            try {
                ...
                if (result.success) {
                    ...
                    if(result.data.length === 0) {
                        itemsHtml = '<tr><td colspan="5" class="text-center py-4" style="color:var(--warning); font-weight:500;">No active brand catalogs assigned to this account asset line.</td></tr>';
                    } else {
                        result.data.forEach(item => {
                            itemsHtml += `<tr>
                                <td class="font-monospace text-muted">${item.barcode}</td>
                                <td><strong class="text-white">${item.item_description}</strong></td>
                                <td><span class="badge-role">${item.location_code}</span></td>
                                <td>
                                    <span class="text-decoration-line-through text-muted small me-2">${item.was_price}</span> 
                                    <strong class="text-success">${item.now_price}</strong>
                                </td>
                                <td class="text-muted">${item.expires_at}</td>
                            </tr>`;
                        });
                    }
        ...
        function cancelUpload() {
            ...
            if (currentRole === 'admin') document.getElementById('adminUploadMerchant').value = '';
        }
        ...
        async function executeUpload() {
            ...
            let targetMerchant = currentUserName;
            if (currentRole === 'admin') targetMerchant = document.getElementById('adminUploadMerchant').value;
            try {
                ...
                if (result.success) {
                    ...
                    else { document.getElementById('adminUploadMerchant').value = ''; }
                }
            } catch (err) { showTempAlert('uploadAlert', `Error: ${err.message}`, false); } finally { confBtn.disabled = false; confBtn.innerText = "Overwrite Engine Stack"; pendingCSVData = null; }
        }
